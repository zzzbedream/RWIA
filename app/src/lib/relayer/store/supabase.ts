 ...
 *
 * Uses the `claim_next_intent(shard, claimed_by)` SQL function (see
 * `supabase/migrations/20260516000000_init_rwia.sql`) which wraps a
 * `SELECT … FOR UPDATE SKIP LOCKED` so multiple relayer processes can
 * poll the same shard concurrently without two workers ever claiming
 * the same job.
 *
 * Schema mapping:
 *   IntentJob.intent.*  →  intents columns (hex addresses stored as bytea)
 *   IntentJob.signature →  intents.signature (bytea)
 *   IntentJob.intentHash → intents.intent_hash (bytea, unique)
 *   IntentJob.status / paid / shardIndex / dlqReason map 1:1
 */
...
export class SupabaseJobStore implements JobStore {
    ...
    async put(job: IntentJob): Promise<IntentJob> {
        // Idempotent insert keyed on intent_hash. If a row already exists for
        // this hash, return the existing job — the on-chain replay guard is
        // the source of truth so we never double-submit.
    ...
    async release(intentHash: string, _claimedBy: string): Promise<IntentJob | undefined> {
        // Two-phase: payment webhook calls this. We atomically flip paid=true
        // ONLY if not already paid (defensive against duplicate webhooks).
        const {data, error} = await this.client
            .from("intents")
    ...
    async claimNext(shardIndex: number, claimedBy: string): Promise<IntentJob | undefined> {
        // Call the Postgres function that wraps FOR UPDATE SKIP LOCKED.
...
/** Convert a `0x…` hex string to a bytea representation Supabase accepts. */
function toBytea(hex: string): string {
    // supabase-js sends bytea as `\x<hex>` literal when targeting Postgres.
    // The supabase REST API also accepts hex with `0x` prefix; we standardize.
    return hex.startsWith("0x") ? hex : `0x${hex}`;
}

function fromBytea(value: string | null): `0x${string}` | undefined {
    if (!value) return undefined;
    // Supabase returns bytea as `\\x<hex>` by default; also handle plain hex.
...
// Re-exports to silence unused-import warning when this file is built but
// the running mode is in-memory.
