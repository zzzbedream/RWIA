        ...
        <main className="min-h-screen bg-black text-white px-6 py-12">
            <div className="max-w-3xl mx-auto space-y-8">
                <header className="flex items-center justify-between">
                    <div>
                        <h1 className="text-3xl font-bold tracking-tight">Buy NFT with fiat</h1>
                        <p className="text-white/60 text-sm mt-1">
                            Sign an EIP-712 intent. After your off-chain payment clears, the Keeper
                            delivers the NFT directly to your wallet.
                        </p>
                    </div>
                    <ConnectWallet />
                </header>
                ...
                {!aggregatorOk ? (
                    <div className="rounded-2xl border border-yellow-500/30 bg-yellow-500/[0.08] p-6">
                        <div className="font-semibold mb-2">Set NEXT_PUBLIC_AGGREGATOR_ADDRESS</div>
                        <div className="text-sm text-white/70">
                            Configure the deployed aggregator address in <code>app/.env.local</code>.
                        </div>
                    </div>
                ) : wrongChain ? (
                    <div className="rounded-2xl border border-amber-500/30 bg-amber-500/[0.08] p-6 space-y-3">
                        <div className="font-semibold">
                            Your wallet is on chain {chainId ?? "?"}, but the contract lives on
                            chain {REQUIRED_CHAIN_ID}.
                        </div>
                        <div className="text-sm text-white/70">
                            EIP-712 signatures must be produced on the same chain the contract is
                            deployed to. Use the orange <strong>Switch to Ronin</strong> button in
                            the header (top-right) — it will prompt your wallet to switch (and add
                            the network if needed).
                        </div>
                        <div className="text-xs font-mono text-amber-300/80">
                            Detected wallet chainId: {chainId ?? "unknown"} ·
                            required: {REQUIRED_CHAIN_ID}
                        </div>
                    </div>
                ) : (
                    <section className="space-y-5 rounded-3xl border border-white/10 bg-white/[0.02] p-8">
                        <Field label="NFT contract (address or .ron)">
                            <RnsAddressInput
                        ...
                        <div className="grid grid-cols-2 gap-4">
                            <Field label="Payment token (ERC-20 used off-chain)">
                                <input
                                    value={paymentToken}
                        ...
                        <Field label="Amount (will be charged off-chain)">
                            <input
                                value={amount}
