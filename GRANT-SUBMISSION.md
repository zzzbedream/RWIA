...
Comparte tu pantalla con https://ronin-waypoint-intent-aggregator-rw.vercel.app/app
  - "Esta es la app deployada en Vercel, contra Ronin Mainnet."
  - "Conecté Ronin Wallet con una cuenta vacía — cero balance, cero RON."
  - "Voy a comprar el TestNFT id 4 — el Keeper ya tiene el inventario
     listo en mainnet."
  - Llena el form rápido (nftContract + tokenId 4).
  ...
  - "El NFT ahora está en mi wallet."
...
[01:45 – 02:00]  Cierre
"El código está abierto en github.com/zzzbedream/RWIA. Las direcciones
de mainnet, la tx de demo, y la URL de producción están en el README.
Gracias."
```
...
```
Website:           https://ronin-waypoint-intent-aggregator-rw.vercel.app
App:               https://ronin-waypoint-intent-aggregator-rw.vercel.app/app
Health check:      https://ronin-waypoint-intent-aggregator-rw.vercel.app/api/health
GitHub:            https://github.com/zzzbedream/RWIA
Whitepaper:        https://github.com/zzzbedream/RWIA/blob/main/HOW-IT-WORKS.md
Security audit:    https://github.com/zzzbedream/RWIA/blob/main/SECURITY.md
Load test report:  https://github.com/zzzbedream/RWIA/blob/main/LOAD-TEST.md
Risk mitigations:  https://github.com/zzzbedream/RWIA/blob/main/RISK-MITIGATIONS.md
Business model:    https://github.com/zzzbedream/RWIA/blob/main/BUSINESS-MODEL.md
Sourcify (Proxy):  https://sourcify.dev/server/v2/contract/2020/0x2D3E5B0dEE29eA5641e1b0Bce08a8A7C5fF82BE5
Sourcify (Impl):   https://sourcify.dev/server/v2/contract/2020/0x1e2cA35cc4CC6036FcCAac4B84F4825dA95e1479
Sourcify (Timelock): https://sourcify.dev/server/v2/contract/2020/0x7862E95164a9F5a9A87Ce9019fD73b6Fe3004123
Ronin Explorer:    https://app.roninchain.com/address/0x2D3E5B0dEE29eA5641e1b0Bce08a8A7C5fF82BE5
Demo tx (E2E):     https://app.roninchain.com/tx/0x1d3b706913db0191c411835417e02306b9368d2c1e035a9e2c560eb6d092ab58
Twitter/X:         <tu cuenta>
Discord:           <tu server>
...
8. Open-source repo (commit history shows every architectural decision):
   https://github.com/zzzbedream/RWIA

== PUBLIC OBSERVABILITY ENDPOINTS (live after Vercel deploy) ==

9. GET ronin-waypoint-intent-aggregator-rw.vercel.app/api/health  →  per-keeper RON balance, shard count, paused state, structured status (ok / degraded / critical) with HTTP 200 / 207 / 503 for alert managers.

10. GET ronin-waypoint-intent-aggregator-rw.vercel.app/api/diagnose?nftContract=0x...&tokenId=N  →  pre-flight check that the keeper holds the NFT, the approval is granted, the role is correct, and the contract is unpaused. Returns a `blockers[]` array operators can render directly.
...
| Contract verified on-chain (Sourcify) | ✅ **3/3 contratos `match`** en sourcify.dev (Proxy + Impl + Timelock) |
| Use Ronin Wallet | ✅ First-class detection via `window.ronin` |
| Use Ronin Waypoint | ✅ SDK `@sky-mavis/waypoint` v4.2.2 integrado |
| Use Ronin Name Service (RNS) | ✅ `lib/rns.ts` con fallback |
| Use Ronin Safe (multisig) | ⏳ Timeline documentado en `SECURITY.md §6.2` (30 días post-grant) |
| Frontend deployed (Vercel/etc) | ✅ https://ronin-waypoint-intent-aggregator-rw.vercel.app |
| Mobile UX | ✅ `/app` responsive (sm: breakpoints, header stacking, form grid colapsable) |
| i18n | ✅ EN/ES toggle en vivo (`/app` header → `LocaleSwitch`) |
| Load test | ✅ 400/400 requests OK contra prod (`LOAD-TEST.md`) |
| Open-source repo | ✅ https://github.com/zzzbedream/RWIA (MIT, full docs) |
| Security: tests + audit | ✅ 19 Foundry tests + SECURITY.md + Slither en CI |
| Health endpoints live | ✅ `/api/health` + `/api/diagnose` returning 200 |
| Supabase JobStore en prod | ✅ Postgres + `claim_next_intent` RPC con SKIP LOCKED |
| CSP + security headers | ✅ `next.config.ts`: CSP completo + 5 headers (X-Frame, XSS, etc.) |
| dApp directory PR | ⏳ Post-grant approval |
...
| Bloqueante | Estado |
|---|---|
| 1. Repo público en GitHub | ✅ https://github.com/zzzbedream/RWIA |
| 2. Frontend desplegado | ✅ https://ronin-waypoint-intent-aggregator-rw.vercel.app |
| 3. Contratos verificados en explorer | ⏳ Sourcify pending (no bloquea grant — código y bytecode son públicos en repo y on-chain) |
| 4. Ronin Safe creado + handoff ejecutado | ⏳ Opcional pre-grant; HandoffToMultisig.s.sol listo |
| 5. dApp directory PR abierto | ⏳ Post-grant approval |
| 6. Loom grabado (NFT #4) | ⏳ Vos — usá DEMO-RUNBOOK.md |
| 7. Smoke E2E live (NFT #3) | ⏳ Vos — usá DEMO-RUNBOOK.md |

Smoke + Loom son lo único que falta para el click de submit.
